...
func ParseCommentForm(r *http.Request) (models.Comment, error) {
	...
	comment.Image, err = utils.ValidateAndSaveImage(r, "comment", strconv.FormatInt(int64(comment.ID)+1, 10))
	if err != nil {
		return comment, err
	}
	return comment, nil
...
func CreateComment(w http.ResponseWriter, r *http.Request, db *sql.DB, userId int) {
	...
	if len(strings.TrimSpace(comment.Content)) == 0 || len(strings.TrimSpace(comment.Content)) > 2500 {
		fmt.Println("hhhhhh")
